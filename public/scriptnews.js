...
window.addEventListener("DOMContentLoaded", async () => {
  ...
  for (let i = antalNyheder - 7; i < antalNyheder; i++) {
    ...
    document.getElementById(`headnews${i - (antalNyheder - 7)}`).innerHTML =
      headnews[i];
    document.getElementById(`imgnews${i - (antalNyheder - 7)}`).innerHTML =
      '<img src="' + imgnews[i] + '">';
    document.getElementById(`newspaper${i - (antalNyheder - 7)}`).innerHTML =
      newspaper[i];
    document.getElementById(`linknews${i - (antalNyheder - 7)}`).innerHTML =
      '<a href="' + urlnews[i] + '">Læs mere</a>';
  }
